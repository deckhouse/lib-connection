...
func (p *SSHProvider) NewAdditionalClient(context.Context) (connection.SSHClient, error) {
	...
	if len(p.switches) > 0 {
		lastSwitch := p.switches[len(p.switches)-1]
		sess = lastSwitch.Session
...
func (p *SSHProvider) SwitchToDefault(ctx context.Context) (connection.SSHClient, error) {
	...
	return p.SwitchClient(ctx, p.initSession, p.initPrivateKeys)
}
...
func (t *tunnel) Up(context.Context) error {
	return nil
}
