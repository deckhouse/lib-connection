...
func TestCommandCombinedOutput(t *testing.T) {
	test := tests.ShouldNewTest(t, "TestCommandCombinedOutput")
...
func TestCommandRun(t *testing.T) {
	test := tests.ShouldNewTest(t, "TestCommandRun")
...
func TestCommandStart(t *testing.T) {
	test := tests.ShouldNewTest(t, "TestCommandStart")
...
func TestCommandSudoRun(t *testing.T) {
	test := tests.ShouldNewTest(t, "TestCommandRunSudo")

	container := tests.NewTestContainerWrapper(t, test, tests.WithNoPassword())
	keys := container.AgentPrivateKeys()

	// starting openssh container with password auth
	containerWithPass := tests.NewTestContainerWrapper(
		t,
		test,
		tests.WithPassword(tests.RandPassword(12)),
		tests.WithConnectToContainerNetwork(container),
	)
	keysContainerWithPass := containerWithPass.AgentPrivateKeys()

	sessionWithoutPassword := tests.Session(container)

	sessionWithValidPass := tests.Session(containerWithPass)

	// client with wrong sudo password
	sessionWithInvalidPass := tests.Session(containerWithPass, func(input *session.Input) {
		input.BecomePass = tests.RandPassword(3)
	})
