...
func TestReverseTunnel(t *testing.T) {
	...
	localServerPort := tests.RandPortExclude([]int{containerPort})

	const response = "Simple response"
	handler := tests.NewSimpleHTTPHandler("/my/action", response)

	tests.MustStartHTTPServer(t, test, localServerPort, handler)
	...
	upTunnelRemoteServerPort := tests.RandPortExclude([]int{containerSSHDPort})
	...
	t.Run("Reverse tunnel from container to host", func(t *testing.T) {
		...
		localInvalidPort := tests.RandInvalidPortExclude([]int{localServerPort})
	...
	t.Run("HealthMonitor test", func(t *testing.T) {
		healthMonitorRemoteServerPort := tests.RandPortExclude([]int{upTunnelRemoteServerPort, containerSSHDPort})
		...
		remoteHealthz := fmt.Sprintf("http://127.0.0.1:%d%s", healthMonitorRemoteServerPort, tests.HealthzPath)
