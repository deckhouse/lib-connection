...
func TestOnlyPreparePrivateKeys(t *testing.T) {
	...
	wrongKeyPath := test.MustCreateTmpFile(t, "Hello world", false, tests.PrivateKeysRoot, "wrong-key")

	validPassword := tests.RandPassword(12)
	keyWithPass, _, err := tests.GenerateKeys(test, validPassword)
	...
	t.Run("OnlyPrepareKeys cases", func(t *testing.T) {
		...
		cases := []testCase{
			...
			},
			{
				...
				keys:    []session.AgentPrivateKey{{Key: keyWithPass, Passphrase: tests.RandPassword(6)}},
				wantErr: true,
...
func TestClientStart(t *testing.T) {
	...
	bastion := tests.NewTestContainerWrapper(
		t,
		test,
		tests.WithConnectToContainerNetwork(container),
		tests.WithUserName(bastionUserName),
		tests.WithAuthSettings(container),
		tests.WithContainerName("bastion"),
	)

	agent := tests.StartTestAgent(t, container)
	...
	cases := []testCase{
		{
			...
			settings: tests.Session(container),
			keys:     make([]session.AgentPrivateKey, 0, 1),
			wantErr:  false,
		},
		{
		...
		{
			title: "No auth",
			settings: tests.Session(container, func(input *session.Input) {
				input.BecomePass = ""
			}),
			keys:     noKeys,
			wantErr:  true,
		...
		{
			title: "Wrong port",
			settings: tests.Session(
				container,
				tests.OverrideSessionWithIncorrectPort(container, bastion),
			),
			keys:     keys,
			wantErr:  true,
		...
		},
		{
			...
			settings: tests.SessionWithBastion(container, bastion),
			keys:     keys,
			wantErr:  false,
			authSock: "",
		},
		...
		},
		{
			...
			settings: tests.SessionWithBastion(container, bastion, func(input *session.Input) {
		...
		},
		{
			...
			settings: tests.SessionWithBastion(
				container,
				bastion,
				tests.OverrideSessionWithIncorrectBastionPort(container, bastion),
			),
			keys:     keys,
			wantErr:  true,
	...
	for _, c := range cases {
		t.Run(c.title, func(t *testing.T) {
			test.SetSubTest(c.title).RunSubTestParallel(t)

			sshSettings := tests.CreateDefaultTestSettingsWithAgent(test, c.authSock)
