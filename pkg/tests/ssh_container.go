...
func (c *SSHContainer) WithVolumes(volumes ...Volume) *SSHContainer {
	if volumes != nil {
		c.settings.AdditionalVolumes = append(c.settings.AdditionalVolumes, volumes...)
	}
...
func (c *SSHContainer) runContainerArgs() []string {
	...
	for _, volume := range c.settings.AdditionalVolumes {
		args = append(args, "-v")
		args = append(args, fmt.Sprintf("%s:%s", volume.Local, volume.Remote))
	}
